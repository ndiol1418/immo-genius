          ...
          <a href="{{ route('annonce', $reco->slug) }}" class="text-decoration-none">
            <div class="card h-100 shadow-sm" style="border-radius:12px;overflow:hidden;transition:transform .2s;" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='none'">
              <div style="position:relative;height:140px;overflow:hidden;">
                <img src="{{ $img ? asset($img->url) : asset('img/logo.png') }}" alt="{{ $reco->name }}" style="width:100%;height:100%;object-fit:cover;">
                <span style="position:absolute;top:6px;left:6px;background:#27E3C0;color:#0d1c2e;font-size:9px;font-weight:700;padding:2px 8px;border-radius:10px;">🤖 Recommandé</span>
              </div>
              <div class="card-body p-2">
                <p class="mb-1 fw-bold" style="font-size:12px;color:#0d1c2e;line-height:1.3;">{{ \Str::limit($reco->name, 30) }}</p>
                <p class="mb-1" style="font-size:13px;color:#27E3C0;font-weight:700;">{{ number_format($reco->prix, 0, ',', ' ') }} CFA</p>
