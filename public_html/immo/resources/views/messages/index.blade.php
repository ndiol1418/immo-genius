...
<style>
  ...
  .conv-avatar { width: 44px; height: 44px; border-radius: 50%; object-fit: cover; background: #2E7D32; display: flex; align-items: center; justify-content: center; color: #fff; font-weight: bold; font-size: 18px; }
</style>
<section class="services section mt-4" style="min-height: 60vh;">
  <div class="container" style="margin-top: 100px; max-width: 700px;">
    <div class="d-flex align-items-center justify-content-between mb-4">
      <h4 class="mb-0">
        <svg xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" viewBox="0 0 24 24" style="color:#2E7D32"><path fill="currentColor" d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2"/></svg>
        Messages
      </h4>
    </div>
    ...
    @else
      @foreach($conversations as $conv)
        ...
        @endphp
        <a href="{{ route('messages.show', $conv->id) }}" class="conv-card {{ $nonLus > 0 ? 'border-start border-3' : '' }}" style="{{ $nonLus > 0 ? 'border-left-color:#2E7D32 !important' : '' }}">
          <div class="d-flex align-items-center" style="gap: 12px;">
            ...
            <div class="flex-grow-1 overflow-hidden">
              ...
              @if($conv->annonce)
                <p style="font-size:11px;color:#2E7D32;margin:0;">{{ $conv->annonce->name }}</p>
              @endif
              @if($dernier)
